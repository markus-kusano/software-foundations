...
(** Which of the following are well-typed elements of [grumble X] for
    some type [X]?
      - [d (b a 5)]
      (b a 5) is a mulble a d takes a muble to a grumble but d requires a type
      so this is not valid.
      - [d mumble (b a 5)]
      This is the same as before but is valid because it passes a type to the
      grumble constructor d.
      - [d bool (b a 5)]
      this is valid for the same reasons as before but instead of producing a
      grumble mumble it produces a grumble bool
      - [e bool true]
      valid it creates a grubmle bool with value true
      - [e mumble (b c 0)]
      valid for the same reasons as before
      - [e bool (b c 0)]
      invalid since (b c 0) is a mumble and not a bool
      - [c] 
      valid this is simply a constructor
(* FILL IN HERE *)
[] *)
...
* Two? I'm not sure what the question is asking... It is not possible to make additional types since this is not a polymorphic type.
(* FILL IN HERE *)
[] *)
...
Proof.
  ...
  Case "l = []".
    simpl. reflexivity.
  Case "l = cons n l'".
    simpl. rewrite rev_snoc. simpl. rewrite IHl'. reflexivity.
Qed.
...
(* i don't know if we have an eplicit greater than function *)
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun n => 
          (andb (negb (ble_nat n 7)) (evenb n))) l.
...
Lemma snoc_map : forall (X Y : Type), forall (f : X -> Y) (l : list X) (n : X),
...
(* constfun 5 returns a function nat -> nat. When this function is run with 
* 99 as the argument, it returns 5. 5 = 5 so this is true. *)
...
Eval compute in (override (constfun true) 3 true) 3 = true.

(* forall bools b when the value to override is 3 and we only pass 2 then we
 * will always get, the value of (constfun b) which is always b *)
...
Proof.
  intros X x1 x2 k1 k2 f H1. intros H2.
...
Lemma fold_length_head : forall (X : Type) (l : list X) (n : X),
...
Proof. 
  ...
  induction l as [| m l'].
    ...
    Case "l = cons m l'".
      simpl. unfold fold_length. simpl. reflexivity. Qed.
...
Proof.
    ...
    induction l as [| n l'].
      ...
      Case "l = cons n l'".
       simpl. rewrite fold_length_head. rewrite IHl'. reflexivity. Qed.
...
(** We can also define [map] in terms of [fold].  Finish [fold_map]
    ...
    (* We X -> Y -> Y for fold, so make the last part of the function do
    * nothing with the second element *)
Definition fold_map {X Y:Type} (f : X -> Y) (l : list X) : list Y :=
  match l with
  | [] => []
  | x :: xs => fold (fun a b => (f a) :: b) l nil
  end.

Eval compute in fold_map evenb [1;2;3;4].
Eval compute in fold_map evenb [].

(* Wow it worked! *)

(** Write down a theorem in Coq stating that [fold_map] is correct,
    and prove it. *)
Lemma fold_map_head_rm : forall (X Y : Type) (f : X -> Y) (l : list X) (n : X),
...
Lemma map_head_rm : forall (X Y : Type) (f : X -> Y) (l : list X) (n : X),
...
Proof.
  ...
  induction l as [| n l'].
    Case "l = []".
      simpl. reflexivity.
    Case "l = cons n l'".
      rewrite fold_map_head_rm. rewrite IHl'. rewrite map_head_rm.
      reflexivity. Qed.
